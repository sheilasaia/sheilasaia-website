...
Be sure to also apply for a NASS API key. We'll use this later but will define it here as a string called `NASS_API_KEY`. You can apply for a NASS API key [here](https://quickstats.nass.usda.gov/api). As with the Census API key, this is unique you so be sure to keep it in a save place.
...
To set the NASS API key your R environment you will have to navigate to the (hidden) .Renviron file on your computer. Then you will have to add `NASS_API_KEY <- "ADD YOUR NASS API KEY HERE"` to that text file. Alternatively, you could also use `sys.setenv()` as indicated below. See [this tutorial](https://rstats.wtf/r-startup.html#renviron) and [this tutorial](https://community.rstudio.com/t/how-to-set-a-variable-in-renviron/5029). You only need to do this once on your computer and will have to readd it if you reformat your computer or reinstall/update R..

Once you've added the NASS API key to your computer's .Renviron file. You will have to run the following code each time you start a new R session because you need to remind R that it has this information available in the .Renviron file.

```{r define NASS api from env, eval=FALSE}
# To add NASS_API_KEY to  your .Renviron file
sys.setenv(NASS_API_KEY <- "ADD YOUR NASS API KEY HERE")

# To check that it is there and load it into your current R session, use this code:
