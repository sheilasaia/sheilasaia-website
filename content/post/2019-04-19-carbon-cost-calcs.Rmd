...
Thanks to friends for their encouragement and to the [#fridaysforthefuture](https://www.fridaysforfuture.org/) movement for inspirting this post.
...
Now that we know the departure and arrival city IATA codes, we can use them to calcualte the distance between RDU and CLT. The `airportr` package also has a function for this.
...
We see there are a few columns here. The first is the year, which ranges from 2015 to 2050. Columns 2 to 5 all represent the cost (in US dollars) of emitting a metric ton of carbon given different future trends. That is, the average (of three models and five climate change scenarios) cost given a 5%, 3%, and 2.5% discount rate for columns 2 through 4, respectively. I'll admit that I don't fully understand the discount rate comes but believe it is used as a way to consider whether people are more likely to value short-term or long-term risks associated with climate change. The larger the discount rate, the less they value long-term risk. In a societal context, the larger discount rate suggests that current generations are valued more than future generations. The report also includes a high impact case that instead of the average uses the 95th percentile cost given a 3% discount rate (rather than the average). The USEPA's [Social Cost of Carbon Technical Report](https://19january2017snapshot.epa.gov/sites/production/files/2016-12/documents/sc_co2_tsd_august_2016.pdf) report says, "...there  is  extensive  evidence  in  the  scientific  and  economic  literature  of  the  potential  for  lower-probability, higher-impact outcomes from climate change, which would be particularly harmful to society and thus relevant to the public and policymakers." Therefore, for this blog post, we'll move forward using the high impact case.
...
Let's plot the data (points) and the model (line).
...
Now let's test out the function for our round-trip flight from RDU to SFO
